...
_TRANSITIONAL_PATTERNS: dict[str, re.Pattern] = {
    ...
    # --- Classic teach-back phrasing ---
    r'tell\s+me\s+(?:back\s+)?in\s+your\s+own\s+words'
    r'|can\s+you\s+(?:explain|describe|tell\s+me|repeat)\s+(?:back\s+)?(?:what|how|what\s+we\s+discussed)'
    r'|in\s+your\s+own\s+words'
    r'|put\s+it\s+into\s+your\s+own\s+words'
    
    # --- Understanding confirmation ---
    r'|just\s+to\s+(?:make\s+sure|confirm)\s+(?:I(?:\'|’)?ve?\s+)?(?:explained|was\s+clear|communicated)\s+clearly'
    r'|to\s+make\s+sure\s+(?:I(?:\'|’)?ve?\s+)?explained\s+(?:this\s+)?clearly'
    r'|checking?\s+(?:your\s+)?understanding'
    r'|make\s+sure\s+you\s+(?:understand|got\s+it|follow)'
    
    # --- Recall / repetition ---
    r'|how\s+would\s+you\s+explain\s+this\s+to\s+someone\s+else'
    r'|can\s+you\s+repeat\s+(?:that|what\s+I\s+just\s+said)'
    r'|what\s+did\s+you\s+understand\s+(?:from\s+this|so\s+far)'
    
    # --- Scenario-based understanding ---
    r'|what\s+(?:would|will)\s+you\s+do\s+if'
    r'|what\s+would\s+you\s+do\s+when'
    r'|how\s+would\s+you\s+(?:respond|handle|know|recognize)'
    r'|what\s+would\s+you\s+do\s+in\s+that\s+case'
    
    # --- Medication / instruction safety checks ---
    r'|how\s+will\s+you\s+(?:take|use|apply|take\s+this\s+medication)'
    r'|when\s+(?:and\s+)?how\s+will\s+you\s+(?:take|use|administer)'
    r'|what\s+is\s+the\s+(?:dose|dosage|schedule)\s+(?:you\s+will\s+follow)'
    
    # --- Confidence / clarity checks ---
    r'|does\s+that\s+make\s+sense'
    r'|is\s+that\s+clear'
    r'|am\s+I\s+being\s+clear'
    r'|do\s+you\s+follow'
    
    # --- Reflection prompts ---
    r'|what\s+will\s+you\s+tell\s+me\s+back'
    r'|how\s+would\s+you\s+summarize'
    r'|what\s+are\s+the\s+key\s+points\s+you\s+remember'
    ...
    r'it(?:\'s|\s+is)\s+(?:completely\s+|totally\s+|very\s+)?'
    r'(?:normal|natural|understandable|okay|ok|common|expected)\s+to\s+'
    r'(?:feel|be\s+feeling|worry|be\s+concerned|be\s+anxious|be\s+scared)'
    
    # --- Sense-making validation ---
    r'|makes?\s+(?:complete\s+|perfect\s+)?sense\s+'
    r'(?:to\s+feel|that\s+you(?:\s+feel|\s+are))'
    r'|it\s+makes\s+sense\s+that\s+you'
    
    # --- Emotional reflection ---
    r'|I\s+can\s+see\s+that\s+you\s+(?:are\s+)?(?:feeling|feel)'
    r'|it\s+sounds\s+like\s+you\s+are\s+feeling'
    r'|it\s+sounds\s+like\s+this\s+is\s+(?:really\s+)?(?:hard|difficult|overwhelming|scary|upsetting)'
    
    # --- Shared human experience ---
    r'|(?:many|most|a\s+lot\s+of)\s+people\s+in\s+(?:your|this)\s+'
    r'(?:situation|position)\s+(?:feel|experience|go\s+through)'
    r'|people\s+(?:often|commonly)\s+feel\s+(?:the\s+same|similar|this\s+way)'
    r'|you\s+are\s+not\s+alone'
    
    # --- Validating emotion explicitly ---
    r'|your\s+(?:concern|worry|fear|anxiety|frustration|stress|feelings?)\s+is\s+'
    r'(?:valid|understandable|normal|completely\s+understandable|very\s+real)'
    
    # --- Emotional naming ---
    r'|feeling\s+(?:worried|anxious|scared|afraid|overwhelmed|stressed|frustrated)'
    r'|you\s+seem\s+(?:worried|anxious|scared|upset|distressed|concerned)'
    
    # --- Permission / normalization ---
    r'|it(?:\'s|\s+is)\s+(?:okay|ok|fine|perfectly\s+(?:okay|normal|natural))\s+to\s+feel'
    r'|anyone\s+would\s+feel\s+(?:that\s+way|like\s+that)'
    r'|that\s+is\s+(?:a\s+)?(?:normal|natural|understandable)\s+reaction'
    
    # --- Trauma-informed framing ---
    r'|given\s+what\s+you(?:\'ve|\s+have)?\s+been\s+through'
    r'|considering\s+what\s+you\s+are\s+going\s+through'
    ...
    # --- Explicit analogy framing ---
    r'like\s+a[n]?\s+\w+(?:\s+\w+){0,3}'
    r'|just\s+like\s+a[n]?\s+\w+(?:\s+\w+){0,3}'
    r'|similar\s+to\s+(?:a[n]?\s+|the\s+)?\w+(?:\s+\w+){0,3}'
    
    # --- "Think of it as" explanations ---
    r'|think\s+of\s+(?:it\s+)?(?:as|like)\s+\w+(?:\s+\w+){0,4}'
    r'|consider\s+(?:it\s+)?(?:as|like)\s+\w+(?:\s+\w+){0,4}'
    
    # --- Imagination / visualization ---
    r'|imagine\s+(?:your|a[n]?\s+|the\s+)?\w+(?:\s+\w+){0,4}'
    r'|picture\s+(?:it|your|a[n]?\s+|the\s+)?\w+(?:\s+\w+){0,4}'
    r'|visualize\s+(?:it|a[n]?\s+|the\s+)?\w+(?:\s+\w+){0,4}'
    
    # --- Functional analogy language ---
    r'|(?:acts?|works?|functions?|behaves?)\s+like\s+a[n]?\s+\w+(?:\s+\w+){0,4}'
    r'|(?:works?|functions?)\s+as\s+(?:a|the)\s+\w+(?:\s+\w+){0,4}'
    
    # --- Possessive metaphors ---
    r'|(?:your|the)\s+\w+(?:\s+\w+){0,3}\s+is\s+like\s+a[n]?\s+\w+(?:\s+\w+){0,4}'
    r'|(?:your|the)\s+\w+(?:\s+\w+){0,3}\s+is\s+similar\s+to\s+(?:a[n]?\s+|the\s+)?\w+'
    
    # --- “It is like…” teaching simplifications ---
    r'|it\s+is\s+like\s+(?:a[n]?\s+|the\s+)?\w+(?:\s+\w+){0,4}'
    r'|it\'?s\s+like\s+(?:a[n]?\s+|the\s+)?\w+(?:\s+\w+){0,4}'
    
    # --- Structural analogy markers ---
    r'|similar\s+to\s+how\s+\w+(?:\s+\w+){0,5}\s+(?:works|functions|operates)'
    r'|just\s+as\s+\w+(?:\s+\w+){0,5}\s+(?:does|works|functions)'
    
    # --- Everyday comparison language ---
    r'|think\s+about\s+(?:it\s+)?like\s+you\s+(?:would|do)\s+\w+'
    r'|you\s+can\s+think\s+of\s+(?:it\s+)?as\s+\w+(?:\s+\w+){0,4}'
    
    # --- Simplified explanatory metaphors ---
    r'|it\s+is\s+as\s+if\s+\w+(?:\s+\w+){0,5}'
    r'|it\s+is\s+as\s+though\s+\w+(?:\s+\w+){0,5}'
    ...
    r'\bstep\s+\d+\b(?:[:\.\)]\s*)?'
    r'|\bstep\s+(?:one|two|three|four|five|six|seven|eight|nine|ten)\b(?:[:\.\)]\s*)?'
    r'|\bstep\s+[ivxlcdm]+\b(?:[:\.\)]\s*)?'  # Roman numerals
    
    # --- Ordered instructions ---
    r'|\bfirst(?:ly)?[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bsecond(?:ly)?[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bthird(?:ly)?[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bfourth(?:ly)?[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bfifth(?:ly)?[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bfinal(?:ly)?[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\blastly[,:]?\s+\w+(?:\s+\w+){0,10}'
    
    # --- Numbered lists ---
    r'|^\s*\d{1,2}[.)]\s+\w+(?:\s+\w+){0,10}'
    
    # --- Bullet / list markers ---
    r'|^\s*(?:[-•*])\s+\w+(?:\s+\w+){0,10}'
    
    # --- Sequential flow language ---
    r'|\bnext[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bthen[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bafter\s+that[,:]?\s+\w+(?:\s+\w+){0,10}'
    r'|\bsubsequently[,:]?\s+\w+(?:\s+\w+){0,10}'
    
    # --- Instructional procedural phrases ---
    r'|\bto\s+(?:start|begin|do\s+this|proceed)\b'
    r'|\bthe\s+first\s+step\s+is\b'
    r'|\bthe\s+next\s+step\s+is\b'
    ...
    r'|johns\s+hopkins\s+medicine'
    r'|nih\.gov'
    
    # --- Global health organizations (patient-facing content) ---
    r'|world\s+health\s+organization|who\.int'
    r'|cdc\.gov'
    r'|ema\.europa\.eu'
    
    # --- National patient orgs ---
    r'|(?:american|british|european|canadian|australian)\s+'
    r'(?:heart|cancer|diabetes|lung|kidney|stroke|medical)\s+'
    r'(?:association|society|foundation|trust|alliance)'
    
    # --- Patient education material language ---
    r'|plain[-\s]?language\s+(?:resource|guide|version|leaflet|summary)'
    r'|easy[-\s]?to[-\s]?read\s+(?:version|guide|leaflet|summary|format)'
    r'|simplified\s+(?:version|guide|summary)'
    
    # --- Patient information documents ---
    r'|(?:patient|consumer|public)\s+'
    r'(?:information|education)\s+'
    r'(?:leaflet|handout|guide|sheet|summary)'
    
    # --- Hospital / clinic education portals ---
    r'|patient\s+education\s+(?:portal|section|materials)'
    r'|health\s+education\s+(?:materials|resources)'
    
    # --- Accessibility / readability indicators ---
    r'|easy\s+read'
    r'|readable\s+version'
    r'|for\s+patients'
    r'|for\s+the\s+public'
    
    # --- Simplified communication formats ---
    r'|question\s+and\s+answer\s+(?:guide|format)'
